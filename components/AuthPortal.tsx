...
import { Key, Shield, TrendingUp, FileText, ArrowRight, CheckCircle2, AlertCircle, ExternalLink } from 'lucide-react';
...
const AuthPortal: React.FC<AuthPortalProps> = ({ onComplete }) => {
  ...
  const handleConnectKey = async () => {
    if (window.aistudio?.openSelectKey) {
      ...
      alert("Key selection is only available in the AI Studio preview environment.");
    }
  };
        ...
        {step === 'auth' && (
          ...
          >
            <div className="text-center mb-10">
              ...
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Join the professional network</p>
            </div>
            ...
            <div className="mt-8 pt-8 border-t border-slate-50 text-center">
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                Already have an account? <button onClick={() => setStep('auth')} className="text-indigo-600 underline">Login</button>
              </p>
            </div>
          </motion.div>
        )}
